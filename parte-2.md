# **Sistemas-y-Procesos-de-Negocio**

---

## Unidades 4 a 6

---

<!-- 1 -->
 <details>
 <summary>
 <h4>
 1. Explique brevemente el concepto de <b>Planeamiento</b> y describa el proceso
 de planeamiento y los niveles que le comprende.
 </h4>
 </summary>

Planear es proyectar un futuro deseado y los medios efectivos para alcanzarlo.
(Russell Ackoff)

    - `Abarque organizacional`:
El Planeamiento estratégico engloba a toda la
organización, mientras que el Planeamiento táctico se limita a un área
específica. En otras palabras, cuantas más funciones de la organización se vean
afectadas por un plan, más estratégico será.

    - Efecto de las decisiones:
Cuanto más largo y irreversible sea el impacto
de un plan, más estratégico se considera. La planeación estratégica se enfoca
en decisiones con efectos duraderos, como la planificación de una nueva planta
industrial o un sistema de distribución de productos, mientras que la planeación
táctica se centra en decisiones de impacto más inmediato, como la producción de
la semana siguiente.

    - Relación con los medios y los fines:
El Planeamiento táctico se ocupa de
definir los medios necesarios para alcanzar objetivos específicos, los cuales
suelen ser establecidos por el nivel directivo en la empresa, es decir, la
planeación estratégica. Por otro lado, la planeación operativa se concentra en
las decisiones que ponen en práctica esos objetivos.

 </details>

---

<!-- 2 -->
 <details>
 <summary>
 <h4>
 2. Describa y diferencie: <b>Planeamiento estretégico, táctico y operativo</b>
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 3 -->
 <details>
 <summary>
 <h4>
 3. Diga los cenceptos de MISIÓN, Visión y Valores, de ejemplos.
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 4 -->
 <details>
 <summary>
 <h4>
 4. ¿Qué se conoce como Cultura Organizacional? Ejemplifique.
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 5 -->
 <details>
 <summary>
 <h4>
 5. ¿Cuál es el concepto de PODER, que tipos de poder conoce?
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 6 -->
 <details>
 <summary>
 <h4>
 6. ¿Cuál es el concepto de Autoridad, qué tipos conoce?
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 7 -->
 <details>
 <summary>
 <h4>
 7. Diferencia Poder de Autoridad y diga qué significa delegar.
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 8 -->
 <details>
 <summary>
 <h4>
 8. Concepto de Liderazgo, qué es un líder y qué tipos de liderazgo conoce.
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 9 -->
 <details>
 <summary>
 <h4>
 9. ¿Qué expresa un organigrama?
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 10 -->
 <details>
 <summary>
 <h4>
 10. ¿Cuáles son los roles gerenciales según Mintzberg?
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 11 -->
 <details>
 <summary>
 <h4>
 11. ¿Qué habilidades deben desarrollar los gerentes?
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 12 -->
 <details>
 <summary>
 <h4>
 12. Explique el proceso de control.
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 13 -->
 <details>
 <summary>
 <h4>
 13. De una definición de control.
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 14 -->
 <details>
 <summary>
 <h4>
 14. ¿Cuáles son las funciones básicas que deben tenerse en cuenta en el proceso
 de control?
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 15 -->
 <details>
 <summary>
 <h4>
 15. ¿Cuáles son los principios que deben cumplir los controles?
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 16 -->
 <details>
 <summary>
 <h4>
 16. ¿Cuáles son los tipos de control?
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 17 -->
 <details>
 <summary>
 <h4>
 17. ¿Qué tareas debe realizarse en el control táctico?
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 18 -->
 <details>
 <summary>
 <h4>
 18. ¿Qué tareas deben realizarse en el control táctico?
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 19 -->
 <details>
 <summary>
 <h4>
 19. ¿Qué tareas deben realizarse en el control estratégico?
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 20 -->
 <details>
 <summary>
 <h4>
 20. Describa el proceso integral del proceso gerencial.
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 21 -->
 <details>
 <summary>
 <h4>
 21. ¿Qué tareas implica el planeamiento estretégico?
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 22 -->
 <details>
 <summary>
 <h4>
 22. ¿Qué tareas implica el planeamiento táctico?
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 23 -->
 <details>
 <summary>
 <h4>
 23. ¿Qué tareas implica el planeamiento operativo?
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 24 -->
 <details>
 <summary>
 <h4>
 24. ¿Qué herramienta conoce para expresar una planificación, ejemplifique?
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 25 -->
 <details>
 <summary>
 <h4>
 25. ¿Cómo se describe a la estructura organizacional?
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 26 -->
 <details>
 <summary>
 <h4>
 26. ¿Cuáles son los elementos de la estructura organizacional?
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 27 -->
 <details>
 <summary>
 <h4>
 27. Dé un concepto de poder.
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 28 -->
 <details>
 <summary>
 <h4>
 28. ¿Cuáles son las categorías y fuentes del poder?
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 29 -->
 <details>
 <summary>
 <h4>
 29. ¿Cuáles son las bases del poder?
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 30 -->
 <details>
 <summary>
 <h4>
 30. Diga a qué se considera Autoridad.
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 31 -->
 <details>
 <summary>
 <h4>
 31. ¿Qué tipos de autoridad pueden darse?
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 32 -->
 <details>
 <summary>
 <h4>
 32. ¿Qué significa departamentalizar?
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 33 -->
 <details>
 <summary>
 <h4>
 33. ¿Cuáles son las formas de departamentalización por propósitos u objetivos?
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 34 -->
 <details>
 <summary>
 <h4>
 34. ¿Qué estrategias se necesitan aplicar para resolver un problema?
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 35 -->
 <details>
 <summary>
 <h4>
 35. Mencione qué herramientas pueden utilizarse para resolver problemas,
 ejemplifique.
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 36 -->
 <details>
 <summary>
 <h4>
 36. Mencione las etapas del Ciclo de vida de los sistemas.
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 37 -->
 <details>
 <summary>
 <h4>
 37. Mencione las diferencias entre Ciclo de Vida de sistemas, Métodos, Técnicas
 , Metodologías y Herramientas.
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 38 -->
 <details>
 <summary>
 <h4>
 38. Diga que es un requerimiento y que significa elicitar un requerimiento.
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 39 -->
 <details>
 <summary>
 <h4>
 39. ¿Cuáles son los prerrequisitos para elicitar requerimientos?
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 40 -->
 <details>
 <summary>
 <h4>
 40. Describa la fase de mantenimiento del software.
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 41 -->
 <details>
 <summary>
 <h4>
 41. Describa la fase de desarrollo de software.
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 42 -->
 <details>
 <summary>
 <h4>
 42. ¿Qué es un sistema de Información Gerencial? Defina.
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 43 -->
 <details>
 <summary>
 <h4>
 43. ¿Cuáles son las Funciones gerenciales? Describa.
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 44 -->
 <details>
 <summary>
 <h4>
 44. ¿Cuáles son los supuestos básicos de la Información como tal?
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 45 -->
 <details>
 <summary>
 <h4>
 45. Mencione algunas técnicas para facilitar la resolución de problemas.
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 46 -->
 <details>
 <summary>
 <h4>
 46. ¿Cuál es la diferencia entre una técnica y una estrategia.
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 47 -->
 <details>
 <summary>
 <h4>
 47. ¿Cuáles son los tipos de usuarios según Senn?
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 48 -->
 <details>
 <summary>
 <h4>
 48. ¿Cuáles son los componentes de un SIG?
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 49 -->
 <details>
 <summary>
 <h4>
 49. Defina un Sistema de ayuda a ejecutivos (SAE).
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 50 -->
 <details>
 <summary>
 <h4>
 50. ¿Qué es un Sistema de apoyo a la decisión (SAD)?
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 51 -->
 <details>
 <summary>
 <h4>
 51. Defina qué es un Sistema de Información de Gestión (SIG).
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 52 -->
 <details>
 <summary>
 <h4>
 52. ¿Qué es un Sistema de procesamiento de transacciones (SPT)?
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 53 -->
 <details>
 <summary>
 <h4>
 53. Describa a un sistema de planificación de recursos (ERP).
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 54 -->
 <details>
 <summary>
 <h4>
 54. ¿Qué se conoce como CRM - Sistemas de Customer Relationship Management?
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 55 -->
 <details>
 <summary>
 <h4>
 55. Describa brevemente a un Sistema de Administración de la cadena de
 suministros (SCM).
 </h4>
 </summary>
 Algo talgoa más
 </details>

---

<!-- 56 -->
 <details>
 <summary>
 <h4>
 56. ¿Qué es un Sistema de Apoyo a las Decisiones (<b>DSS</b> - por sus siglas
 en inglés <b>Decision Support System</b>)?
 </h4>
 </summary>
 Algo talgoa más
 </details>

